...
public class LevelRenderer {
	...
	public static void renderArea(Graphics2D g, Level level, int x0, int y0, int xCam, int yCam, int w, int h) {
		...
		Art.init(null); // Initialize Art resources, passing null for GC in headless mode
		// Pass null for GraphicsConfiguration when potentially running headless
		MarioComponent mc = new MarioComponent(w,h);
		// The LevelScene might not need it if only rendering to a pre-existing Graphics2D
		LevelScene world = new LevelScene(null, mc, 0, 0, 0, w, 0); 
		...
		for (int x = xTileStart; x <= xTileEnd; x++) {
			for (int y = yTileStart; y <= yTileEnd; y++) {
				...
				SpriteTemplate sprite = level.getSpriteTemplate((xCam / MarioLevelViewer.BLOCK_SIZE) + x, y); // Adding
																												// xCam
																												// so
																												// enemies
																												// render
	...
	public static void renderAreaWhiteBack(Graphics2D g, Level level, int x0, int y0, int xCam, int yCam, int w,
			int h) {
		...
		for (int x = xCam / 16; x <= (xCam + width) / 16; x++)
			for (int y = yCam / 16; y <= (yCam + height) / 16; y++) {
				...
				if (((Level.TILE_BEHAVIORS[b & 0xff]) & Level.BIT_ANIMATED) > 0) {
					...
					g.drawImage(Art.level[(b % 16) / 4 * 4 + animTime][b / 16], (x << 4) - xCam, (y << 4) - yCam - yo,
							null);
				}
				/*
				 ...
				 * int animTime = (tick / 3) % 4;
				 * int yo = 0;
				 * if (x >= 0 && y >= 0 && x < level.width && y < level.height) yo =
				 * level.data[x][y];
				 * if (yo > 0) yo = (int) (Math.sin((yo - alpha) / 4.0f * Math.PI) * 8);
				 * g.drawImage(Art.mapSprites[(4 + animTime)][0], (x << 4) - xCam, (y << 4) -
				 * yCam - yo, null);
				 * }
				 */
