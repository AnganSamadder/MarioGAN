...
public class MarioLevelViewer {
	...
	public static void main(String[] args) throws IOException {
		...
		// Initialize without starting the MarioProcess to avoid JFrame creation
		...
		if (args.length > 0) {
			...
			// If the input starts with two square brackets, then it must be an array of
			// arrays,
			// and hence a series of several latent vectors rather than just one. In this
			// case,
			...
			System.out.println("No latent vector provided. Generating level with random vector...");
			// Generate a random latent vector
			int latentDim = Integer.parseInt(Settings.GAN_DIM); // Use dimension from Settings
			double[] randomLatentVector = new double[latentDim];
			Random random = new Random();
			for (int i = 0; i < latentDim; i++) {
				randomLatentVector[i] = random.nextGaussian(); // Generate random numbers from standard normal
																// distribution
			}
			System.out.println("Generated random vector: " + java.util.Arrays.toString(randomLatentVector));
