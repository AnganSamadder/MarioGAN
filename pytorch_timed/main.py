...
if opt.experiment is None:
    # Default to 'samples' subdirectory within the script's directory
...
print(f"Ensuring experiment directory exists: {opt.experiment}")
os.makedirs(opt.experiment, exist_ok=True)

# Define model save directory (relative to script) and create it
model_save_dir = os.path.join(script_dir, 'models')
print(f"Ensuring model save directory exists: {model_save_dir}")
...
for epoch in range(opt.niter):
    ...
    while i < num_batches:#len(dataloader):
        ...
        if gen_iterations % 50 == 0:   #was 500
            ...
            # Commented out image saving to save space
            # plt.imsave('{0}/mario_fake_samples_{1}.png'.format(opt.experiment, gen_iterations), im)
            # Save checkpoint, overwriting the latest (both G and D)
            torch.save(netG.state_dict(), '{0}/netG_latest.pth'.format(opt.experiment))
            torch.save(netD.state_dict(), '{0}/netD_latest.pth'.format(opt.experiment))

    # do checkpointing (REMOVED - handled by periodic save)
    #torch.save(netG.state_dict(), '{0}/netG_epoch_{1}.pth'.format(opt.experiment, epoch))
    #torch.save(netD.state_dict(), '{0}/netD_epoch_{1}.pth'.format(opt.experiment, epoch))

# --- ADDED Final Model Saving --- 
print("Training finished. Saving final models...")
try:
    torch.save(netG.state_dict(), f'{model_save_dir}/netG_final.pth')
    torch.save(netD.state_dict(), f'{model_save_dir}/netD_final.pth')
    print("Final models saved successfully.")
except Exception as e:
    print(f"Error saving final models: {e}")
# --- End Added Final Model Saving ---
