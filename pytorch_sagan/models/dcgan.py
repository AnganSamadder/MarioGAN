...
class DCGAN_D(nn.Module):
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0):
        ...
        while csize > 4:
            ...
            csize = csize / 2
            if pyramid_level == 1:
                main.add_module('self_attention_d_{0}'.format(cndf),
                                SelfAttention(cndf))
            pyramid_level += 1
    ...
    def forward(self, input):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            output, _ = nn.parallel.data_parallel(self.main, input, range(self.ngpu))
        else: 
            features = input
            attention_map = None
            for name, layer in self.main.named_children():
...
class DCGAN_G(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        csize, current_ngf = 4, cngf
        pyramid_level = 0
        while csize < isize//2:
            main.add_module('pyramid_{0}-{1}_convt'.format(current_ngf, current_ngf//2),
                            nn.ConvTranspose2d(current_ngf, current_ngf//2, 4, 2, 1, bias=False))
            main.add_module('pyramid_{0}_batchnorm'.format(current_ngf//2),
                            nn.BatchNorm2d(current_ngf//2))
            main.add_module('pyramid_{0}_relu'.format(current_ngf//2),
                            nn.ReLU(True))
            current_ngf = current_ngf // 2
            csize = csize * 2
            if pyramid_level == 1:
                 main.add_module('self_attention_g_{0}'.format(current_ngf),
                                SelfAttention(current_ngf))
            pyramid_level += 1

        # Extra layers
        for t in range(n_extra_layers):
            main.add_module('extra-layers-{0}_{1}_conv'.format(t, current_ngf),
                            nn.Conv2d(current_ngf, current_ngf, 3, 1, 1, bias=False))
            main.add_module('extra-layers-{0}_{1}_batchnorm'.format(t, current_ngf),
                            nn.BatchNorm2d(current_ngf))
            main.add_module('extra-layers-{0}_{1}_relu'.format(t, current_ngf),
                            nn.ReLU(True))

        main.add_module('final_{0}-{1}_convt'.format(current_ngf, nc),
                        nn.ConvTranspose2d(current_ngf, nc, 4, 2, 1, bias=False))
